...
bool StoryModeScene1::init()
{
    ...
    this->schedule(CC_SCHEDULE_SELECTOR(StoryModeScene1::moveEnemy), 3, CC_REPEAT_FOREVER, 2);
    this->scheduleOnce(CC_SCHEDULE_SELECTOR(StoryModeScene1::initBoss), 30);

    CocosDenshion::SimpleAudioEngine::getInstance()->preloadBackgroundMusic("audio/Level1Music.mp3");
    CocosDenshion::SimpleAudioEngine::getInstance()->playBackgroundMusic("audio/Level1Music.mp3", true);
...
void StoryModeScene1::initEnemy(cocos2d::Sprite* _e, int bMask)
{
    _e->setScale(.35);
...
void StoryModeScene1::update(float delta)
{
    ...
    if (playerHearts < 3 && playerHearts >= 0)
    {
...
void StoryModeScene1::initBossPhysics(cocos2d::Sprite* _b)
{
    _b->setScale(1.5);
...
void StoryModeScene1::initBoss(float dt)
{
    _boss = Sprite::create("Boss1.png");
    CocosDenshion::SimpleAudioEngine::getInstance()->playEffect("audio/BossScream.mp3");
    initBossPhysics(_boss);
    this->addChild(_boss);
    _boss->runAction(MoveBy::create(2, Point(0, -250)));
    this->schedule(CC_SCHEDULE_SELECTOR(StoryModeScene1::bossAttack), 7, CC_REPEAT_FOREVER, 3);
}

void StoryModeScene1::bossAttack(float dt)
{
    ...
    this->unschedule(CC_SCHEDULE_SELECTOR(StoryModeScene1::moveEnemy));
    laser = Sprite::create("Laser.png");
    laser->setScale(.01);
    laser->setAnchorPoint(Vec2(.5, 1));
    laser->setPosition(Point(visibleSize.width / 2, visibleSize.height - (1.4*_boss->getContentSize().height)));
    ...
    auto fullAttack1 = Sequence::create(strecht, rotate1, rotate2, unstrecht, CallFuncN::create(CC_CALLBACK_1(StoryModeScene1::doRemoveFromParent, this)), CallFuncN::create(CC_CALLBACK_0(StoryModeScene1::stopLaser, this)), nullptr);
    laser->runAction(fullAttack1);
}

void StoryModeScene1::stopLaser()
{
...
void StoryModeScene1::levelCompleted()
{
    this->unscheduleAllCallbacks();
    CocosDenshion::SimpleAudioEngine::getInstance()->stopBackgroundMusic();
    CocosDenshion::SimpleAudioEngine::getInstance()->stopAllEffects();
